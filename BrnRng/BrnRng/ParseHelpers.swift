...
extension String {
    func stringBetweenStrings(start : String?, end : String?) -> String? {
        let startRange = (start != nil) ? self.rangeOfString(start!, options: .WidthInsensitiveSearch | .CaseInsensitiveSearch, range: nil, locale: nil) : Range(start: self.startIndex, end: self.startIndex)
        if !startRange!.isEmpty {
            let endRange = (end != nil ? self.rangeOfString(end!, options: .WidthInsensitiveSearch | .CaseInsensitiveSearch, range: nil, locale: nil) : Range(start: self.endIndex, end: self.endIndex))
            if !endRange!.isEmpty {
...
extension TFHppleElement {
    func extractTextAndImages() -> (String?, NSURL?) {
        ...
        return (textContainer != nil ? textContainer!.clean() : nil, imageUrl)
    }
